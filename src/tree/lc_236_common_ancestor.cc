#include "tree/lc_236_common_ancestor.h"
...
TEST_F(LowestCommonAncestorTest, lowestCommonAncestor1) {
  ...
  TreeNode<int>* res2 = exec->lowestCommonAncestor1(root2, root2->left, root2->left->right->left);
  ASSERT_EQ(5, res2->val);
  FreeBinaryTree(root2);

  std::vector<int> nums3 = {1, 2, -1, -1, -1};
  TreeNode<int>* root3 = ToBinaryTree(nums3);
  TreeNode<int>* res3 = exec->lowestCommonAncestor1(root3, root3, root3->left);
  ...
  TreeNode<int>* res4 = exec->lowestCommonAncestor1(root4, root4->right->right, root4->right->right->left->left);
